...
Fixed SR prompt, no text CFG. LQ condition dropout (configurable) preserves baseline generation ability.
"""
...
class AVRestorationConfig(TrainingStrategyConfigBase):
    ...
    cond_mode: Literal["latent", "pixel_wave"] = Field(
        default="latent",
        description="Condition injection mode: 'latent' (VAE-encoded LQ) or 'pixel_wave' (raw pixel/waveform, no LQ VAE)",
    )
...
class AVRestorationStrategy(TrainingStrategy):
    ...
    LQ dropout preserves baseline ability: when LQ is dropped, the step is
    equivalent to standard text-to-AV training. No text CFG — SR is driven
    entirely by LQ condition injection.
    """
    ...
    def prepare_training_inputs(
        ...
        is_pixel_wave = self.config.cond_mode == "pixel_wave"

        if is_pixel_wave:
            # pixel_wave mode: LQ is raw pixel [B, 3, T, H, W], passed directly to PixelCondProj
            lq_cond_raw = batch["lq_pixel"]
        else:
        ...
        if is_pixel_wave:
            # PixelCondProj handles everything: pixel → tokens internally
            video_cond_input = lq_cond_raw
        elif cross_res_sr:
            # Keep LQ as raw 5D [B, C, T, H_lq, W_lq] for CondSRPatchifyProj
            video_cond_input = lq_latents
        else:
            video_cond_input = self._video_patchifier.patchify(lq_latents)
        ...
        if drop_cond:
            video_cond_latent = torch.zeros_like(video_cond_input)
        else:
            video_cond_latent = video_cond_input
            # Optionally add noise to condition (in token space for pixel_wave, latent space otherwise)
    ...
    def _prepare_audio_inputs(
        ...
        if is_pixel_wave:
            # pixel_wave mode: LQ audio is raw waveform, passed directly to WaveCondProj
            lq_audio_cond_raw = batch["lq_waveform"]
            if drop_cond:
                audio_cond_latent = torch.zeros_like(lq_audio_cond_raw)
            else:
                audio_cond_latent = lq_audio_cond_raw
