"""Perceptual losses for distillation training.
...
def _haar_dwt2_spatial(x: Tensor) -> dict[str, Tensor]:
    """Haar wavelet 2D decomposition on spatial dims."""
    if x.shape[-2] % 2 != 0:
        x = x[..., :-1, :]
    if x.shape[-1] % 2 != 0:
        x = x[..., :, :-1]
...
def compute_wavelet_loss(
    ...
    """L1 loss on high-frequency wavelet subbands."""
...
def load_lpips_model(device: torch.device, net: str = "vgg") -> nn.Module:
    ...
    import lpips
    local_dir = os.environ.get("ECHO_SR_LPIPS_DIR")
    if local_dir:
        model_path = os.path.join(local_dir, f"{net}.pth")
...
def compute_lpips_loss(pred: Tensor, target: Tensor, lpips_model: nn.Module) -> Tensor:
    """LPIPS on single frames. Input: [B, C, H, W] in [-1, 1]."""
    return lpips_model(pred.float(), target.float()).mean()


def compute_pixel_temporal_loss(
    pred_frames: list[Tensor], gt_frames: list[Tensor],
) -> Tensor:
    """L1 loss on inter-frame pixel differences: pred diff vs GT diff.

    Args:
        pred_frames: list of [B, C, 1, H, W] decoded frames (sorted by time).
        gt_frames:   list of [B, C, 1, H, W] GT pixel frames (same order).
    Returns:
        Scalar temporal consistency loss.
    """
    if len(pred_frames) < 2:
        return torch.tensor(0.0, device=pred_frames[0].device, requires_grad=True)
    losses = []
    for i in range(len(pred_frames) - 1):
        pred_diff = pred_frames[i + 1] - pred_frames[i]
        gt_diff = gt_frames[i + 1] - gt_frames[i]
        losses.append(F.l1_loss(pred_diff, gt_diff))
    return sum(losses) / len(losses)
