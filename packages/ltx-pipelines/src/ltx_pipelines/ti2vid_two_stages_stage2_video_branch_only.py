...
from ltx_core.model.transformer import LTXV_MODEL_COMFY_RENAMING_MAP, LTXVideoOnlyModelConfigurator
...
class VideoOnlyDiffusionStage(DiffusionStage):
    """
    Same runtime as DiffusionStage, but the transformer is instantiated as a
    true video-only DiT (no audio branch modules are created).
    """
...
class TI2VidTwoStagesStage2VideoBranchOnlyPipeline:
    """
    Two-stage ablation pipeline:
    - Stage 1: normal audio-video generation
    - Stage 2: uses a true video-only DiT, so the audio branch is removed at
      model construction time rather than only passing audio=None

    Final audio is reused from stage 1 unchanged.
    """
    ...
    def __init__(
        ...
        self.stage_2 = VideoOnlyDiffusionStage(
            checkpoint_path,
            self.dtype,
            self.device,
    ...
    def __call__(  # noqa: PLR0913
        ...
        video_state, audio_state_stage_1 = self.stage_1(
        ...
        logger.info("Stage 2 ablation enabled: loading a true video-only DiT (audio branch removed).")
        video_state, _ = self.stage_2(
            denoiser=SimpleDenoiser(v_context=v_context_p, a_context=None),
        ...
        decoded_audio = self.audio_decoder(audio_state_stage_1.latent)
...
def main() -> None:
    ...
    pipeline = TI2VidTwoStagesStage2VideoBranchOnlyPipeline(
