...
class TI2VidTwoStagesStage2VideoOnlyPipeline:
    """
    Two-stage text/image-to-video pipeline variant for ablation:
    - Stage 1: normal AV generation at half resolution
    - Stage 2: video SR/refinement only (audio input to stage-2 DiT is disabled)

    Final audio is reused from stage 1 so you can compare whether removing stage-2
    audio conditioning changes the 1K SR video quality.
    """
    ...
    def __call__(  # noqa: PLR0913
        ...
        logger.info("Stage 2 ablation enabled: stage-2 DiT runs with video only (audio=None).")
...
def main() -> None:
    ...
    pipeline = TI2VidTwoStagesStage2VideoOnlyPipeline(
