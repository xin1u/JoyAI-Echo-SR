...
class LTXModel(torch.nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        # Condition projection layers for additive SR injection (initialized later via init_cond_proj)
        self._cond_video_proj: torch.nn.Linear | None = None
        self._cond_audio_proj: torch.nn.Linear | None = None
    ...
    def init_cond_proj(self, init_std: float = 1e-6) -> None:
        """Initialize additive condition projection layers for SR/restoration.
        Creates cond_video_proj and cond_audio_proj that map LQ condition latents
        to the hidden space, element-wise added after patchify_proj.
        Near-zero init ensures training starts from a stable pretrained baseline.
        """
        if self.model_type.is_video_enabled():
            self._cond_video_proj = torch.nn.Linear(
                self.patchify_proj.in_features, self.inner_dim, bias=True
            )
            torch.nn.init.normal_(self._cond_video_proj.weight, std=init_std)
            torch.nn.init.zeros_(self._cond_video_proj.bias)
            self.cond_video_proj = self._cond_video_proj

        if self.model_type.is_audio_enabled():
            self._cond_audio_proj = torch.nn.Linear(
                self.audio_patchify_proj.in_features, self.audio_inner_dim, bias=True
            )
            torch.nn.init.normal_(self._cond_audio_proj.weight, std=init_std)
            torch.nn.init.zeros_(self._cond_audio_proj.bias)
            self.cond_audio_proj = self._cond_audio_proj

        self._rebuild_preprocessors_with_cond()

    def init_cond_sr_proj(
        self,
        lq_h: int,
        lq_w: int,
        hq_h: int,
        ...
        """Initialize learned cross-resolution SR patchify projection.

        Creates a CondSRPatchifyProj that maps raw 5D LQ latent directly to
        HQ token space without interpolation. Coexists with cond_audio_proj
        (audio path unchanged).
        """
        from ltx_core.model.transformer.cond_sr_patchify import CondSRPatchifyProj

        if self.model_type.is_video_enabled():
            self._cond_sr_video_proj = CondSRPatchifyProj(
                in_channels=self.patchify_proj.in_features,
                inner_dim=self.inner_dim,
                lq_h=lq_h,
                lq_w=lq_w,
                hq_h=hq_h,
            ...
            CondSRPatchifyProj.init_near_identity(self._cond_sr_video_proj, proj_std=init_std)
            self.cond_sr_video_proj = self._cond_sr_video_proj
        ...
        self._rebuild_preprocessors_with_cond(use_sr_proj=True)

    def _rebuild_preprocessors_with_cond(self, use_sr_proj: bool = False) -> None:
        """Rebuild args preprocessors to include condition projection layers."""
        cross_pe_max_pos = None
        if self.model_type.is_video_enabled() and self.model_type.is_audio_enabled():
        ...
        if self.model_type.is_video_enabled() and self.model_type.is_audio_enabled():
            video_cond_proj = (
                getattr(self, "_cond_sr_video_proj", None)
                if use_sr_proj
                else getattr(self, "_cond_video_proj", None)
            )
            self.video_args_preprocessor = MultiModalTransformerArgsPreprocessor(
                patchify_proj=self.patchify_proj,
                adaln=self.adaln_single,
            ...
            self.audio_args_preprocessor = MultiModalTransformerArgsPreprocessor(
                ...
                caption_projection=getattr(self, "audio_caption_projection", None),
                prompt_adaln=getattr(self, "audio_prompt_adaln_single", None),
                cond_proj=self._cond_audio_proj,
            )
