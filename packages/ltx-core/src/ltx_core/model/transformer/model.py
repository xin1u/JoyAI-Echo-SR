...
class LTXModel(torch.nn.Module):
    ...
    def _process_transformer_blocks(
    ...
    ) -> tuple[TransformerArgs, TransformerArgs, torch.Tensor | None]:
        """Process transformer blocks for LTXAV.

        If ``capture_layer`` is given (1-indexed), the video hidden state that is
        the OUTPUT of that block is captured and returned as the third element
        (used to harvest an intermediate DiT deep feature for the CVAE decoder).
        """
        ...
        for idx, block in enumerate(self.transformer_blocks):
            ...
            # Capture the output of the requested (1-indexed) layer.
            if capture_layer is not None and video is not None and (idx + 1) == capture_layer:
                captured = video.x

        return video, audio, captured
    ...
    def forward(
        ...
        Args:
            return_hidden_states: when True, also return the video/audio last
                transformer-block hidden states (pre-proj_out, shape
                (B, num_tokens, inner_dim)) as a third element. Used by the
                conditional VAE decoder trainer to harvest the DiT deep feature.
        Returns:
            Processed output tensors. If return_hidden_states is True, returns
            (vx, ax, {"video": video_out.x, "audio": audio_out.x}).
        """
        ...
        # Process transformer blocks
        video_out, audio_out, captured_hidden = self._process_transformer_blocks(
            video=video_args,
            audio=audio_args,
            perturbations=perturbations,
        ...
        if return_hidden_states:
            # When capture_layer is set, return that intermediate layer's output
            # for video; otherwise fall back to the last block's hidden state.
            hidden = {
                "video": (
                    captured_hidden
                    if captured_hidden is not None
                    else (video_out.x if video_out is not None else None)
                ),
                "audio": audio_out.x if audio_out is not None else None,
            }
            return vx, ax, hidden
        return vx, ax
